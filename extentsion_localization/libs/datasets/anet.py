    ...
    def __getitem__(self, idx):
        ...
        if self.use_hdf5:
            ...
            with np.load(filename) as data:
                # Qui dobbiamo indovinare la chiave. Spesso è 'features', 'arr_0' o il nome del video.
                # Proviamo a prendere il primo array disponibile se non conosciamo il nome:
                key = list(data.keys())[0] 
                feats = data[key].astype(np.float32)
        print(f"DEBUG: Caricate le features per il video {video_item['id']} con shape {feats.shape}")
        if feats.shape[0] == 2304:
                # È nel formato (Canali, Tempo), lo giriamo in (Tempo, Canali)
