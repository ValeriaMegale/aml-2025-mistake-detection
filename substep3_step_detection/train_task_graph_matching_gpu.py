...
class TaskGraphMatchingDataset(Dataset):
    """Dataset per Task Graph Matching con perception features"""
    ...
    def __init__(
        ...
        video_ids,
        annotation_map,
        recording_to_activity,
        activity_to_taskgraph,
        task_graph_dir,
        text_encoder,
        precomputed_text_emb=None
    ):
        self.samples = []
        ...
        for vid in tqdm(video_ids, desc="Preparing dataset", leave=False):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--visual_dim', type=int, default=768,
                        help='Visual embedding dimension: 768 for perception, 1024 for omnivore')
...
# python substep3_step_detection/train_task_graph_matching_gpu.py --epochs 30 --batch_size 16

# Con gradient accumulation (per GPU con poca memoria):
# python substep3_step_detection/train_task_graph_matching_gpu.py --batch_size 8 --accumulation_steps 2
