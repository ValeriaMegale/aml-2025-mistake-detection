...
# Script per eseguire tutti i training con diverse combinazioni
# Esegue max 2 training in parallelo per non sovraccaricare la GPU
...
# Configurazioni: (pos_weight, lr, weight_decay)
CONFIG_OLD_PW="2.5"
CONFIG_OLD_LR="1e-3"
CONFIG_OLD_WD="1e-3"
...
for variant in "${VARIANTS[@]}"; do
    for backbone in "${BACKBONES[@]}"; do
        wait_for_slot
        pid=$(run_training "$variant" "$backbone" "$CONFIG_OLD_LR" "$CONFIG_OLD_WD" "OLD")
        PIDS+=($pid)
        sleep 2  # Piccolo delay per evitare race conditions
    done
done

# Attendi che tutti i training OLD siano completati
echo ""
...
for variant in "${VARIANTS[@]}"; do
    for backbone in "${BACKBONES[@]}"; do
        wait_for_slot
        pid=$(run_training "$variant" "$backbone" "$CONFIG_NEW_LR" "$CONFIG_NEW_WD" "NEW")
        PIDS+=($pid)
        sleep 2  # Piccolo delay per evitare race conditions
    done
done

# Attendi che tutti i training NEW siano completati
echo ""
